...
/* Payload every OOB round-trip below wraps: a SERVER_PROBE message header
 * (message_id 1, response_id 0) and one TLV (type 1, 4 bytes of value). */
static const uint8_t oob_payload[] = { 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
                                       0x00, 0x00, 0x01, 0x00, 0x04, 0xde, 0xad, 0xbe, 0xef };
                         ...
                         struct tls_auth_standalone *tas_server)
{
     ...
     * carries. */
    struct buffer buf =
        tls_wrap_oob_standalone(&tas_client->tls_wrap, tas_client, &sid, &payload, P_CONTROL_OOB_V1);
 ...
 * decrypts the payload, and returns VERDICT_VALID_OOB_WKC_V1. */
static void
test_oob_wkc_v1_roundtrip(void **ut_state)
{
    ...
    assert_int_equal(verdict, VERDICT_VALID_OOB_WKC_V1);
