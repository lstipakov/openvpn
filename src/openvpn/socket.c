...
link_socket_init_phase1(struct context *c, int sock_index, int mode)
{
    ...
    else if (mode == LS_MODE_UDP_ADOPT)
    {
        ...
        /* Take the probe's lookup as the remote list, so replies are checked
         * against the addresses that were probed rather than against a second
         * lookup that may return others; resolve_remote() then resolves nothing.
         * Pin the exact address that answered, the one the cookie was minted
         * for; resolve_remote() preserves a defined actual. */
        struct link_socket_addr *lsa = sock->info.lsa;
        if (lsa->remote_list && !c->options.resolve_in_advance)
        {
            freeaddrinfo(lsa->remote_list);
        }
        lsa->remote_list = c->c2.oob_probe_remote_list;
        c->c2.oob_probe_remote_list = NULL; /* ownership moves with the socket */
        lsa->current_remote = lsa->remote_list;
        for (struct addrinfo *ai = lsa->remote_list; ai; ai = ai->ai_next)
        {
            if (ai->ai_family != c->c2.oob_probe_remote.addr.sa.sa_family)
            {
                continue;
            }
            struct openvpn_sockaddr entry = { 0 };
            memcpy(&entry, ai->ai_addr, min_int((int)ai->ai_addrlen, (int)sizeof(entry)));
            if (addr_port_match(&entry, &c->c2.oob_probe_remote))
            {
                lsa->current_remote = ai; /* a failed attempt moves on from here */
                break;
            }
        }
        CLEAR(lsa->actual);
        lsa->actual.dest = c->c2.oob_probe_remote;
    }
