 ...
 *  Copyright (C) 2021 Antonio Quartulli <a@unstable.cc>
...
/* following are the DCO APIs used to control the driver */
int
dco_new_peer(struct tuntap *tt, unsigned int peerid, int sd,
             struct sockaddr *localaddr, struct sockaddr *remoteaddr,
             struct in_addr *remote_in4, struct in6_addr *remote_in6);
int
ovpn_set_peer(struct tuntap *tt, unsigned  int peerid,
              unsigned int keepalive_interval, unsigned int keepalive_timeout);
int
dco_del_peer(struct tuntap *tt, unsigned int peerid);
...
int
dco_swap_keys(struct tuntap *tt, unsigned int peerid);
