...
struct probe_ctx
{
    ...
    /* Every probe sent, in order; a probe's message_id is its index + 1. An
     * address several entries resolve to is probed once per round, and its
     * reply is credited to each of them. */
    struct oob_probe_send *sends;
    int n_sends;
    int max_sends;
...
static bool
oob_probe_sent_this_round(const struct probe_ctx *pc, const struct openvpn_sockaddr *addr)
{
    for (int i = pc->round_start; i < pc->n_sends; i++)
    {
        if (addr_port_match(&pc->sends[i].dest, addr))
        {
            return true;
        }
    }
...
{
    /* CE_DISABLED entries are skipped by next_connection_entry(), so probing
     * them would rank a remote we are never going to use. */
                       ...
                       struct oob_probe_result *results, int n)
{
    ...
    if (!oob_client_reply_read(&buf, &response_id, &reply))
    {
        return;
    }

    /* Reject spoofed replies: the reply must echo our probe's session id. */
    if (!session_id_equal(&reply.peer_session_id, pc->client_sid))
    {
        return;
    }
...
oob_probe_receive_slice(const struct probe_ctx *pc, const struct timeval *deadline,
                        const struct oob_probe_target *targets, struct oob_probe_result *results,
                        int n, int outstanding)
{
    while (true)
    {
        ...
        for (int i = 0; i < PROBE_AF_COUNT; i++)
        {
            ...
            if (len > 0)
            {
                oob_probe_handle_reply(pc, data, len, &from, targets, results, n);
            }
        }
                  ...
                  struct oob_probe_result *results, int n, const struct signal_info *sig)
{
    ...
    for (int slice = 0; slice < slices; slice++)
    {
        ...
        if (oob_probe_receive_slice(pc, &deadline, targets, results, n, want)
            || sig->signal_received)
        {
        ...
        if (slice + 1 < slices)
        {
            oob_probe_resend_unanswered(pc, targets, results, n);
        }
    }
}
