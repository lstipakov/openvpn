 ...
 * @p dests / @p destlens arrays are gc-allocated, sized to the remote's resolved
 * address count. */
struct probe_target
{
                  ...
                  const struct probe_target *targets, struct oob_probe_result *results, int n)
{
    ...
    for (int slice = 0; slice < slices; slice++)
    {
        ...
        if (oob_probe_receive_slice(sd, &deadline, client_sid, wrap, targets, results, n, want))
        {
            return; /* all answered */
        }
...
client_probe_and_order_remotes(struct context *c)
{
    ...
    for (int i = 0; i < l->len; i++)
    {
        ...
        for (const struct addrinfo *a = ai; a; a = a->ai_next)
        {
            socklen_t destlen = oob_probe_make_dest(a, sock_af, &t->dests[t->n_dests]);
            if (destlen > 0)
            {
    ...
    if (sent_count > 0)
    {
        oob_probe_collect(sd, &probe, &client_sid, &tas->tls_wrap, targets, results, l->len);
    }
    openvpn_close_socket(sd);
