...
int
dco_install_key(struct tls_multi *multi, int keyid,
...

{
    ...
    return dco_new_key(multi->dco, multi->peer_id, keyid, slot,
...
void
init_key_dco_bi(struct tls_multi *multi, const struct key2 *key2,
                int key_direction, unsigned int key_id,
                const char *ciphername, bool server)
{
    struct key_direction_state kds;
    key_direction_state_init(&kds, key_direction);

    dco_install_key(multi, key_id,
