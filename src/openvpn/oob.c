...
oob_effective_margin(const struct oob_probe_result *r, int client_margin)
{
    ...
    return (int)r->reply.max_latency_diff; /* else what the fastest remote advertised */
}
...
oob_weighted_order(const struct oob_probe_result *results, int *idx, int m, int64_t (*rng)(void))
{
    for (int pos = 0; pos < m; pos++)
    {
        ...
        /* Move the chosen entry to pos, keeping the rest in RTT order (RFC 2782
         * removes the selected entry from the list). */
        int sel = idx[chosen];
        for (int k = chosen; k > pos; k--)
        {
            idx[k] = idx[k - 1];
        }
        idx[pos] = sel;
                       ...
                       struct gc_arena *gc)
{
    ...
    int nc = 0;
    int nn = 0;
    /* The fastest remote's margin sets the band; inclusive, so the fastest is
     * always a candidate, also when it advertises 0. */
    const unsigned int margin = (unsigned int)oob_effective_margin(&run[0], client_margin);
    for (int k = 0; k < m; k++)
    {
